...
mod.color.init = function()
    mod.color.hud.lerp_ratio = mod:get("hud_color_lerp_ratio") and mod:get("hud_color_lerp_ratio") or 0.5
    mod.color.hud.monster_only_if_alive = mod:get("monsters_hud_only_if_alive")
    ...
    for _, lvl in pairs(mod.priority_levels) do
        ...
        --for i, col_code in pairs(notif_color_lvl) do
        for i=1, 4 do
            local color_code = 0
...
mod.interesting_breed_names.init()
    Partially (see -.sort()) initialise the other elements. These are currently not dependent on mod settings in any way, and as such, this method only needs to be called once.
mod.interesting_breed_names.inverted_table
    mod.interesting_breed_names.inverted_table[breed_name] = true if the breed_name is trackable by the mod, nil otherwise.
...
mod.tracked_units.init = function()
    ...
    for _, breed_name in pairs(mod.interesting_breed_names.array) do
        mod.tracked_units.priority_levels[breed_name] = tostring(mod:get(breed_name.."_priority"))
...
mod.tracked_units.refresh_unit_count = function(breed_name)
    ...
    -- If breed_name is a monster, check whether the unit count changed from zero to non-zero or vice-versa
...
local get_breed_presentation_name = function(breed)
    ...
    local this_color = get_breed_color(clean_breed_name)
...
local add_new_notif = function(breed_name, event)
    ...
    local message = mod:localize(event.."_message_simple", display_name) -- Could make another loc to avoid showing "x1"
...
mod.update = function(dt)
    ...
    for breed_name, nb_of_deaths in pairs(nb_of_deaths_per_breed) do
        -- NB: This will work well when (or if) we get to grouping notifications together
        if mod.tracked_units.notif_breeds_inverted_table[breed_name] then
